...
def check_oov(format_sql_final, output_vocab, schema_tokens):
  for sql_tok in format_sql_final.split():
    if not (sql_tok.lower() in schema_tokens or sql_tok.lower() in output_vocab):
...
def read_chase(spider_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  interaction_list = {}
...
def read_csgsql(spider_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  interaction_list = {}
...
def preprocess(dataset, remove_from=False):
  ...
  if dataset == 'spider':
    ...
    csgsql_dir = 'data/csgsql/'
    database_schema_filename = 'data/csgsql/tables.json'
    output_dir = 'data/csgsql_data'
    if remove_from:
  ...
  if dataset == 'spider':
    ...
    interaction_list = read_csgsql(csgsql_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from)
...
if __name__ == '__main__':
  ...
  parser.add_argument("--dataset", choices=('spider', 'sparc', 'cosql', 'chase', 'csgsql'), default='csgsql')
