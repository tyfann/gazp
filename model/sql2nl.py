...
class Module(Base):
    ...
    def __init__(self, args, ext):
        ...
        self.conv = converter.Converter(tables=getattr(args, 'tables', 'data/chase/tables'), db=getattr(args, 'db', 'data/database'))
    ...
    def run_gen_on_split(self, size, SQLSampler, db_split, fout, args=None, save=True):
        ...
        self.conv = self.nl2sql.conv = converter.Converter(tables=getattr(args, 'tables', 'data/csgsql/tables'), db=getattr(args, 'db', 'data/database'))
